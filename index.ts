...
<body>
  ...
  <div class="badge">🌍 39 Corridors</div>
  ...
  <div class="endpoint">💳 POST /api/pay — FX payment (AC2 authorized)</div>
  <div class="endpoint">💳 POST /api/orchestrate — FX routing</div>
  <hr/>
...
app.get("/api/rates", (c) => {
    ...
    return c.json({
        ...
        rates,
        note: "Use POST /api/pay (AC2 authorized) to execute a route"
    });
});
...
app.post("/api/authorize", async (c) => {
    ...
    if (outcome.kind === "response") {
        return c.json({
            ...
            message: "AC2 approval granted. Proceed to /api/pay.",
            protocol: "AC2 on Algorand"
        });
    }
    return c.json({ approved: false, reason: "AC2 approval declined" }, 403);
});

// AC2-authorized payment endpoint (Algorand native)
app.post("/api/pay", async (c) => {
    const { from, to, amount, approval_id } = await c.req.json();
    if (!from || !to || !amount) {
        return c.json({ error: "Missing: from, to, amount" }, 400);
    }
    if (!approval_id || !approval_id.startsWith("bercy_")) {
        return c.json({ error: "Invalid approval_id. Call /api/authorize first." }, 401);
    }
    const rates = getAllRates();
    ...
    return c.json({
        ...
        route: {
            ...
            protocols: ["AC2", "Algorand"],
            approval_id,
            payment_status: "AC2_AUTHORIZED"
        }
    });
});

// FX orchestration endpoint (open)
...
serve({ fetch: app.fetch, port: PORT }, () => {
    console.log(`🦅 Bercy FX Orchestrator (AC2 + Live Rates) running on port ${PORT}`);
});
